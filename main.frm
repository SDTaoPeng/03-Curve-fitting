...
Begin VB.Form FrmDataCH 
   Caption         =   "基于主曲线方法的水轮机特性曲线数值拟合(重画过程)         王宜怀编    "
   ClientHeight    =   10665
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   14985
...
Private Sub CmdCH_Click()  '重画
   ...
   TLine = 0
    ...
    Loop
    Close #1                               '关闭文件
   p = Mid$(TextLine(2), 5, 4)         '隐含层(中间层)单元个数:p
   TxtState.Text = TxtState.Text & "  中间层单元个数p=" & p     '过程信息
   ...
   '连接权w(i,j)   输入层到中间层
    ...
    '---------------------------------------------------------------
       ...
       For d1 = Tfx1 To Tfxm Step 0.0002     '重画
             aa(1) = d1
            ...
            '   S函数计算输出层各单元的输出响应cc(t)
             ...
             b = cc(1)
             b1 = cc(2)
             a = (b + 1 - (1 - b * b) ^ 0.5) / (2 * b)        '见文档“数学表达式推导.docx”
             a1 = (b1 + 1 - (1 - b1 * b1) ^ 0.5) / (2 * b1)   '见文档“数学表达式推导.docx”
             
             'a1 = (2 * cc(2) - 1) / (2 * cc(2) * cc(2) - 2 * cc(2) + 1)
                          
             Xpoint = 0
             Ypoint = 0
               
             Xpoint = ((a * 2 - 1) * xymax + Sumx) * PQ11 * 1000 + X0
             Ypoint = ((a1 * 2 - 1) * xymax + Sumy) * Pn11 * 100 + Y0
             
             PicData.PSet (Xpoint, Ypoint), vbBlue
             'PicData.PSet (Xpoint, Ypoint), vbRed
